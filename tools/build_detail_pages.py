...
def slug_image(section, slug, item=None):
    ...
    if item:
        ...
        if og:
            url = og if og.startswith("http") else f"{ORIGIN}/{og.lstrip('/')}"
            return (url, 1200, 630)
...
def render_page(section, slug, item, prev_item, next_item):
    ...
    og_image_url, og_w, og_h = slug_image(section, slug, item)
    title_ja = item["title"]["ja"]
    excerpt_ja = item.get("excerpt", {}).get("ja", "")
    ...
    # SEO overrides from new schema fields
    ...
    if video:
        if "youtube.com" in video or "youtu.be" in video:
            ...
            if "v=" in video:
                ...
                vid_id = video.split("youtu.be/")[1].split("?")[0]
            if vid_id:
                video_html = f'<div class="detail-video"><iframe src="https://www.youtube.com/embed/{vid_id}" title="{title_attr}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen loading="lazy"></iframe></div>'
        elif video.endswith((".mp4", ".webm", ".mov")):
    ...
    # Custom share text if set
    share_text = item.get("share_text", "")
    share_text_enc = urllib.parse.quote(share_text, safe="") if share_text else share_title_enc
...
<section class="share-bar" aria-label="Share this article">
  <div class="container share-bar__inner">
    <span class="share-bar__label">Share</span>
    <div class="share-bar__buttons">
      <!-- Tier 1: International business / professional -->
      <a class="share-btn share-btn--linkedin" href="https://www.linkedin.com/sharing/share-offsite/?url={share_url_enc}" target="_blank" rel="noopener" aria-label="Share on LinkedIn" title="LinkedIn">
        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
      </a>
      <a class="share-btn share-btn--x" href="https://x.com/intent/tweet?text={share_text_enc}&url={share_url_enc}" target="_blank" rel="noopener" aria-label="Share on X" title="X (Twitter)">
        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.259 5.636L18.244 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77z"/></svg>
      </a>
      <a class="share-btn share-btn--facebook" href="https://www.facebook.com/sharer/sharer.php?u={share_url_enc}" target="_blank" rel="noopener" aria-label="Share on Facebook" title="Facebook">
        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
      </a>
      <!-- Tier 2: Curated content / thought-leader communities -->
      <a class="share-btn share-btn--reddit" href="https://www.reddit.com/submit?url={share_url_enc}&title={share_text_enc}" target="_blank" rel="noopener" aria-label="Share on Reddit" title="Reddit">
      ...
      <a class="share-btn share-btn--hackernews" href="https://news.ycombinator.com/submitlink?u={share_url_enc}&t={share_text_enc}" target="_blank" rel="noopener" aria-label="Submit to Hacker News" title="Hacker News">
        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M0 24V0h24v24H0zM6.951 5.896l4.112 7.708v5.064h1.583v-4.972l4.148-7.799h-1.749l-2.457 4.875c-.372.745-.688 1.434-.688 1.434s-.297-.708-.651-1.434L8.831 5.896h-1.88z"/></svg>
      </a>
      <a class="share-btn share-btn--bluesky" href="https://bsky.app/intent/compose?text={share_text_enc}%20{share_url_enc}" target="_blank" rel="noopener" aria-label="Share on Bluesky" title="Bluesky">
      ...
      <a class="share-btn share-btn--threads" href="https://www.threads.net/intent/post?text={share_text_enc}%20{share_url_enc}" target="_blank" rel="noopener" aria-label="Share on Threads" title="Threads">
        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12.186 24h-.007c-3.581-.024-6.334-1.205-8.184-3.509C2.35 18.44 1.5 15.586 1.472 12.01v-.017c.03-3.579.879-6.43 2.525-8.482C5.868 1.205 8.621.024 12.199 0h.014c2.814.024 5.129.807 6.875 2.332C20.906 3.768 21.925 5.78 22.172 8.2l.014.135h-3.443l-.012-.107c-.25-1.83-.945-3.18-2.11-4.12-1.168-.942-2.775-1.433-4.776-1.457h-.012c-2.412.017-4.301.761-5.619 2.21C4.88 6.309 4.218 8.405 4.196 11.992c.02 3.589.685 5.686 2.017 7.13 1.318 1.449 3.208 2.193 5.618 2.21 1.895-.02 3.384-.54 4.428-1.546.961-.929 1.521-2.264 1.663-3.97H12.18v-3.27h8.938l-.025.198c-.31 3.168-1.37 5.522-3.15 7.003-1.68 1.397-3.922 2.1-6.665 2.1-.013 0-.027 0-.04-.001h-.052z"/></svg>
      </a>
      <!-- Tier 3: Save / curate -->
      <a class="share-btn share-btn--pocket" href="https://getpocket.com/save?url={share_url_enc}&title={share_text_enc}" target="_blank" rel="noopener" aria-label="Save to Pocket" title="Pocket">
        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.813 10.259l-5.461 5.39a1.943 1.943 0 01-1.358.556 1.967 1.967 0 01-1.358-.556l-5.461-5.39a1.95 1.95 0 01.001-2.774 1.985 1.985 0 012.81-.001l4.008 3.954 4.007-3.954c.78-.769 2.04-.769 2.81 0a1.954 1.954 0 01.002 2.775zM12.005 0H1.842C.825 0 0 .816 0 1.821v9.219C0 17.643 5.371 23 12.005 23 18.638 23 24 17.643 24 11.04V1.821C24 .816 23.184 0 22.167 0H12.005z"/></svg>
      </a>
      <a class="share-btn share-btn--pinterest" href="https://pinterest.com/pin/create/button/?url={share_url_enc}&description={share_text_enc}" target="_blank" rel="noopener" aria-label="Pin on Pinterest" title="Pinterest">
      ...
      <!-- Tier 4: Messaging -->
      <a class="share-btn share-btn--telegram" href="https://t.me/share/url?url={share_url_enc}&text={share_text_enc}" target="_blank" rel="noopener" aria-label="Share on Telegram" title="Telegram">
      ...
      <a class="share-btn share-btn--whatsapp" href="https://wa.me/?text={share_text_enc}%20{share_url_enc}" target="_blank" rel="noopener" aria-label="Share on WhatsApp" title="WhatsApp">
      ...
      <a class="share-btn share-btn--email" href="mailto:?subject={share_text_enc}&body={share_url_enc}" aria-label="Share by email" title="Email">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
      </a>
      <!-- Tier 5: Regional (Asia) -->
      <a class="share-btn share-btn--line" href="https://social-plugins.line.me/lineit/share?url={share_url_enc}" target="_blank" rel="noopener" aria-label="Share on LINE" title="LINE">
      ...
      <button class="share-btn share-btn--wechat" type="button" onclick="document.getElementById('wechat-qr').classList.toggle('is-open')" aria-label="Share on WeChat (show QR code)" title="WeChat (QR)">
        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M8.691 2C4.262 2 .677 5.106.677 8.94c0 2.21 1.21 4.18 3.094 5.46l-.78 2.32 2.71-1.36c.97.19 1.95.31 2.99.31.18 0 .36-.01.55-.02-.12-.42-.18-.85-.18-1.3 0-3.27 2.83-5.92 6.31-5.92.18 0 .36.01.54.02-.5-3.05-3.6-5.45-7.22-5.45zM5.79 6.6c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm5.81 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm9.51 8.91c0-3.05-3.05-5.53-6.45-5.53s-6.45 2.48-6.45 5.53 3.05 5.53 6.45 5.53c.71 0 1.39-.11 2.04-.3l1.86 1-.51-1.7c1.85-1.04 3.06-2.7 3.06-4.53zm-8.4-1.42c-.42 0-.77-.35-.77-.77s.35-.77.77-.77.77.35.77.77-.35.77-.77.77zm3.86 0c-.42 0-.77-.35-.77-.77s.35-.77.77-.77.77.35.77.77-.35.77-.77.77z"/></svg>
      </button>
      <!-- Utility -->
      <button class="share-btn share-btn--copy" type="button" onclick="navigator.clipboard.writeText(window.location.href).then(()=>{{this.classList.add('is-copied');setTimeout(()=>this.classList.remove('is-copied'),1800)}})" aria-label="Copy link" title="Copy link">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
      </button>
    </div>
  </div>
  <div id="wechat-qr" class="wechat-qr-modal" onclick="if(event.target===this)this.classList.remove('is-open')">
    <div class="wechat-qr-modal__inner">
      <h4>WeChatでスキャン</h4>
      <img alt="WeChat QR code for this page"
           src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data={share_url_enc}">
      <p class="wechat-qr-modal__hint">WeChatのスキャナーで読み取って共有してください</p>
      <button type="button" class="btn btn-sm" onclick="document.getElementById('wechat-qr').classList.remove('is-open')">閉じる</button>
    </div>
  </div>
</section>
